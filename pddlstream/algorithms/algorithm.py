...
def parse_constants(domain, constant_map):
    ...
    for constant in domain.constants:
        if constant.name.startswith(Object._prefix):  # TODO: check other prefixes
            raise NotImplementedError(
                "Constants are not currently allowed to begin with {}".format(
                    Object._prefix
                )
            )
        ...
        obj_from_constant[constant.name] = Object(
            value, name=constant.name
        )  # TODO: remap names
    ...
    for name in constant_map:
        for constant in domain.constants:
            ...
            raise ValueError(
                "Constant map value {} not mentioned in domain :constants".format(name)
            )
...
def check_problem(domain, streams, obj_from_constant):
    for action in domain.actions + domain.axioms:
        for p, c in Counter(action.parameters).items():
            if c != 1:
                raise ValueError(
                    "Parameter [{}] for action [{}] is not unique".format(
                        p.name, action.name
                    )
                )
    ...
    for stream in streams:
        ...
        for fact in facts:
            ...
            if name not in domain.predicate_dict:
                ...
                len(get_args(fact)) != domain.predicate_dict[name].get_arity()
            ):  # predicate used with wrong arity: {}
                print(
                    "Warning! predicate used with wrong arity in stream [{}]: {}".format(
                        stream.name, fact
                    )
                )
    ...
    if undeclared_predicates:
        print(
            "Warning! Undeclared predicates: {}".format(sorted(undeclared_predicates))
...
def parse_streams(
    ...
    for lisp_list in stream_iter:
        ...
        if name == ":stream":
            externals = [
                parse_stream(
                    lisp_list, procedure_map, procedure_info, use_unique=use_unique
                )
            ]
        elif name == ":rule":
